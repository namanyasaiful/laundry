...
<div class="main-content" style="min-height: 659px;">
    <section class="section">
        <div class="card">
            ...
            <form action="{{ route('member.update', $member[0]->id)}}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="form-group">
                        <form>
                        <label>Nama</label>
                        <input type="text" name="nama" class="form-control" value="{{ $member[0]->nama }}">
                    </div>
                    <div class="form-group">
                        <form>
                        <label>Alamat</label>
                        <input type="text" name="alamat" class="form-control" value="{{ $member[0]->alamat }}">
                    </div>
                    <div class="form-group">
                        <label>Telepon</label>
                        <input type="text" name="tlp" class="form-control" value="{{ $member[0]->tlp }}">
                    </div>
                </div>
                <div class="card-footer text-right">
                    <a class="btn btn-danger" href="{{route('member.index')}}">kembali</a>
